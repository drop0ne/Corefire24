...
void MyConsoleAPI_extension::generateMainMenu(const std::vector<int>& stateData) {
	...
	print("Option", stateData.at(Options), " 8 ", stateData.at(ProgramID), "-", stateData.at(Symbols), " Show Mathematical Proofs\n", stateData.at(Program));
	print("Option", stateData.at(Options), " 9 ", stateData.at(ProgramID), "-", stateData.at(Symbols), " Calculator\n", stateData.at(Program));
	print("Option", stateData.at(Options), " X or EXIT ", stateData.at(ExitProgramID), "-", stateData.at(Symbols), " To Quit Program\n", stateData.at(ExitProgram));
}
	...
	do {
		...
		try {
			...
			if (std::all_of(input.begin(), input.end(), ::isdigit)) {
				returnValue = std::stoi(input); // Convert string to integer
				return returnValue; // Return if valid number
			}
			else {
